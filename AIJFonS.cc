...
void mapa_post(int primera_id, vector<int> & mapa,
    ...
    void logica_heli(HeliInfo & h) {
        if (h.target.x == -1) h.target = Posicio(36,40);
        ...
        for (int i = -2; i <= 2; ++i) {
            for (int j = -2; j <= 2; ++j) {
                ...
                if( id > 0 and dades(id).equip != equip) ++enemics;
            }
        }

        if (enemics > 4 and h.da.napalm == 0) ordena_helicopter(h.id, NAPALM);
...
bool visitar_posicio_bfs_helis(int instruccio, int id, vector<bool> & visitat, queue<int> &q, int &inici) {
    if (not visitat[id] and validHelis[id % nMapa]) {
        visitat[id] = true;
        q.push(id);
        if (id == inici) {
...
int bfs_heli(Posicio posInici, int orientacioInici, Posicio posDesti) {
    ...
    while (!q.empty()) {
        ...
        Posicio p = Posicio(pos.x + dx[o], pos.y + dy[o]);
        if (valid(p))
            if (visitar_posicio_bfs_helis(AVANCA1, po2id(p, o), visitat, q, inici)) {
                return AVANCA1;
            }
        ...
        if (visitar_posicio_bfs_helis(AVANCA1, po2id(pos, (o + 1) % 4), visitat, q, inici)) {
                return RELLOTGE;
        }
