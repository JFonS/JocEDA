...
struct PLAYER_NAME: public Player {
	...
	void init_mapes() {
		...
		for (int p = 0; p < nPosts; ++p) {
			for (int i = 0; i < 60; ++i) {
				for (int j = 0; j < 60; ++j) {
					cerr << " " << distanciesSoldats[p][c2id(i, j)];
				}
				cerr << endl;
			}
			cerr << "-----------------------------" << endl;
	...
	void init_soldat(Soldat &s) {
		...
		priority_queue<P, vector<P>, greater<P> > q = priority_queue<P,
				vector<P>, greater<P> >();
		//priority_queue<pair<int,int>> q = priority_queue<pair<int,int>,vector<pair<int,int>>,greater<pair<int,int>>();
	...
	void mou_helicopter(const HeliInfo &h) {
		ordena_helicopter(h.id,
				mapesHelis[h.post][po2id(h.da.pos, h.da.orientacio)]);
	}
